<?php
class functions {
    public static function uploadImg($fileObj, $dir)
    {
        $path_parts = pathinfo($_FILES[$fileObj]["name"]);
        $imagefilename = functions::convertname($path_parts['filename']);
        $imageFileType = $path_parts['extension'];
        //    $path_parts = pathinfo($_FILES['hinh_anh']["name"]);
        //    echo $path_parts['dirname'], "\n";
        //    echo $path_parts['basename'], "\n";
        if ($_FILES[$fileObj]["size"] > 5000000)
            $return =  1;
        ...
        //     $return =  2;
            ...
            $newfile =  $imagefilename . '.' . $imageFileType;
            $target = $dir . $newfile;
            if (file_exists($target)) {
                $newfile = $imagefilename . '_' . time() . '.' . $imageFileType;
                $target = $dir . $newfile;
            }
            if (move_uploaded_file($_FILES[$fileObj]["tmp_name"], $target))
                $return = $newfile;
            else
                $return = 3;
        }
